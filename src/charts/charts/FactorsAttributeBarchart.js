/**
 * @fileoverview Defines a FactorAttributeBarchart component that visualizes the distribution of attributes within a dataset.
...
export default class FactorsAttributeBarchart {
    ...
    constructor(element, dataStore, height = 500, isZoomable = true) {
        ...
        this.distribution = this.factors.map((factor, i) => ({
            label: `${i + 1}`,
            value: factor.attributes.length
        }));
    ...
    draw() {
        ...
        this.chartGroup.selectAll("g.bar")
            ...
            .on("mouseover", (event, d) => {
                this.tooltip
                    .style("display", "block")
                    .html(`Number of attributes: ${d.value}`);
