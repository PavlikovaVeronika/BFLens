...
export default class FactorAttributeStackedBarchart {
  ...
  constructor(element, factorIdx, dataStore, height = 500, isZoomable = true, selectAll = true) {
    ...
    let distribution = this.dataStore.getFactorAttributeDistribution(this.factorIdx);
  ...
  draw() {
    ...
    const factorAttributeDistribution = this.dataStore.getFactorAttributeDistribution(this.factorIdx);
    const attributeDistribution = this.dataStore.getAttributeDistribution();

    let merged = attributeDistribution
      .map((d, i) => ({
        label: d.label,
        overall: d.value,
        factor: factorAttributeDistribution[i].value
      }));
